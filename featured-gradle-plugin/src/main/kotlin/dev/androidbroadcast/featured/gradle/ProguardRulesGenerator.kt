 ...
 * ```proguard
 * -assumevalues class dev.androidbroadcast.featured.generated.GeneratedLocalFlagExtensionsFeatureUiKt {
...
public object ProguardRulesGenerator {
     ...
     * [ExtensionFunctionGenerator.localFileName]: the Kotlin compiler uses the file name
     * (without `.kt`) plus the `Kt` suffix as the JVM class name for top-level declarations.
     *
     * [packageName] must be the effective package of the local-flags section (see
     * `generation { packageName }` in the DSL) — the rules target the extensions file
     * emitted into that package, so a mismatch silently disables release-build DCE.
     *
    ...
    public fun generate(
        ...
        modulePath: String,
        packageName: String = ConfigParamGenerator.DEFAULT_PACKAGE,
    ): String {
        val localEntries = entries.filter { it.isLocal && jvmType(it.type) != null }
        if (localEntries.isEmpty()) return ""

        val className = "$packageName.${ExtensionFunctionGenerator.localFileName(modulePath).removeSuffix(".kt")}Kt"
