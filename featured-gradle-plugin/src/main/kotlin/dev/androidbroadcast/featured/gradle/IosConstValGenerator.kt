...
public object IosConstValGenerator {
     ...
     *
     * [packageName] is the effective package of the local-flags section (see
     * `generation { packageName }` in the DSL).
     *
     ...
     * `const val` does not support enum types.
     *
     * [packageName] must match the package passed to [generate] — expect/actual pairs
     * must live in the same package.
     *
     * Returns a blank string when [entries] is empty or all entries are enums.
     */
    public fun generateExpect(
        entries: List<LocalFlagEntry>,
        packageName: String = ConfigParamGenerator.DEFAULT_PACKAGE,
    ): String {
        val eligible = entries.filterNot { it.isEnum }
